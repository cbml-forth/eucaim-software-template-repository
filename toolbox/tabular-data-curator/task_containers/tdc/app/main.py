...
def data_curator(path, imputation_method_id, outlier_detection_method_id, sim_method_id, output_dir="results"):
    ...
    temp_excel_path = os.path.join(output_dir, "temp_data.xlsx")
    ...
    path_f_c = os.path.join(output_dir, fname_c)
    path_f_c_v2 = os.path.join(output_dir, fname_c_v2)
    path_f_e = os.path.join(output_dir, fname_e)
    path_f_sim_corr = os.path.join(output_dir, fname_sim_corr)
    path_f_sim_lex = os.path.join(output_dir, fname_simlex)
    path_f_json = os.path.join(output_dir, fname_json)
...
def resolve_input_file(input_path):
    """Accepts either a direct path to a dataset file, or a directory
    containing exactly one supported dataset file (.csv/.json/.xlsx/...)."""
    if os.path.isdir(input_path):
        candidates = sorted(
            f for f in os.listdir(input_path)
            if allowed_file(f) and os.path.isfile(os.path.join(input_path, f))
        )
        if len(candidates) == 0:
            raise ValueError(
                f"No supported dataset file (.csv/.json/.xlsx) found in directory: {input_path}"
            )
        if len(candidates) > 1:
            raise ValueError(
                f"Expected exactly one dataset file in {input_path}, found {len(candidates)}: {candidates}"
            )
        return os.path.join(input_path, candidates[0])

    if not os.path.isfile(input_path):
        raise ValueError(f"Input path does not exist: {input_path}")
    if not allowed_file(input_path):
        raise ValueError(f"Unsupported file extension for input: {input_path}")
    return input_path


def main():
    parser = argparse.ArgumentParser(
        description="TDC (Tabular Data Curation) - data quality evaluation, curation and similarity detection for tabular datasets."
    )
    parser.add_argument(
        "-i", "--input", required=True,
        help="Path to the input dataset (.csv, .json or .xlsx), or a directory containing exactly one such file."
    )
    parser.add_argument(
        "-o", "--output-dir", required=True,
        help="Directory where the generated reports will be written."
    )
    parser.add_argument(
        "--outlier-method", type=int, default=1, choices=range(1, 8),
        help="Outlier detection method: 1=z-score, 2=z-score (mod.), 3=IQR, 4=Grubb's test, "
             "5=Local Outlier Factor, 6=Isolation Forests, 7=Isolation Forests (mod.). Default: 1."
    )
    parser.add_argument(
        "--similarity-method", type=int, default=1, choices=range(1, 6),
        help="Similarity detection method: 1=Spearman, 2=Pearson, 3=Kendall's tau, "
             "4=Covariance, 5=None. Default: 1."
    )
    parser.add_argument(
        "--imputation-method", type=int, default=1, choices=range(1, 5),
        help="Data imputation method: 1=Average/median, 2=Random, 3=Zeros, 4=None. Default: 1."
    )
